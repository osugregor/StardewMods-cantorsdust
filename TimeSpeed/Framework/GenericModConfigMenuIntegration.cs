...
internal static class GenericModConfigMenuIntegration
{
    ...
    *********/
    ...
    public static void Register(IManifest manifest, IModRegistry modRegistry, IMonitor monitor, Func<ModConfig> getConfig, Action reset, Action save)
    {
        ...
        ConstructMenu(api, manifest, getConfig, reset, save);
    }

    public static void ConstructMenu(IGenericModConfigMenuApi api, IManifest manifest, Func<ModConfig> getConfig, Action reset, Action save)
    {
        if (!Context.IsMainPlayer)
        {
            api.AddParagraph(manifest, I18n.Config_FarmhandInfo_Desc);
        }
        else
        {
            // general options
